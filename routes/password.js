...
const email = process.env.EMAIL_USERNAME_PROVIDER;
const pass = process.env.EMAIL_PASSWORD_PROVIDER;

const smtpTransport = nodemailer.createTransport({
  host: process.env.EMAIL_HOST_PROVIDER,
  port: process.env.EMAIL_PORT_PROVIDER,
...
router.post('/forgot-password', asyncMiddleware(async (req, res, next) => {
  ...
  const data = {
    to: user.email,
    from: process.env.EMAIL_USERNAME_PROVIDER,
